...
local rawset, rawget, Vector, Angle, worldToLocal, LocalToWorld, FrameTime, EffectData = rawset, rawget, Vector, Angle, WorldToLocal, LocalToWorld, FrameTime, EffectData
local table_insert = table.insert
local mesh_Begin, mesh_End, mesh_Position, mesh_TexCoord, mesh_Normal, mesh_AdvanceVertex = mesh.Begin, mesh.End, mesh.Position, mesh.TexCoord, mesh.Normal, mesh.AdvanceVertex
local render_SetMaterial, render_SetColorModulation = render.SetMaterial, render.SetColorModulation
local math_Round, math_atan2, math_abs, math_sin, math_floor, math_ceil, math_max, math_min, math_Clamp = math.Round, math.atan2, math.abs, math.sin, math.floor, math.ceil, math.max, math.min, math.Clamp
local math_rad = math.pi / 180
local math_deg = 180 / math.pi
local _mvec = Vector()
local _mvec_set, _mvec_add, _mvec_dot, _mvec_rotate, _mvec_length, _mvec_distance = _mvec.Set, _mvec.Add, _mvec.Dot, _mvec.Rotate, _mvec.Length, _mvec.Distance
local _mang = Angle()
local _mang_forward, _mang_right, _mang_up = _mang.Forward, _mang.Right, _mang.Up
local _mmat = Matrix()
local _mmat_setTranslation, _mmat_setAngles = _mmat.SetTranslation, _mmat.SetAngles
local debugtype = {
	trace = {
		enabled = true,
		update = function(self, ent, reset)
			if reset or not self.data[ent] then self.data[ent] = {} end
...
function ENT:_DakVar_CHANGED(name, old, new)
	...
	if self._DakVar_ONCHANGED then self:_DakVar_ONCHANGED(name, old, new) end
end

function ENT:Think()
	local selfTbl = self:GetTable()

	if selfTbl.dak_resetGearbox and selfTbl.dak_resetGearbox < CurTime() then
		selfTbl.dak_resetGearbox = nil
		selfTbl.dak_vehicleMode = self:GetVehicleMode()
		self:setup_gearbox(selfTbl.dak_vehicleMode)
		return
	end

	self:update_gearbox(selfTbl.dak_vehicleMode, selfTbl)
end
...
function ENT:OnRemove()
	...
	timer.Simple(0, function()
		if (self and self:IsValid()) or not snapshot then return end
...
function ENT:csents_create()
	if not self.dak_csents then self.dak_csents = {} end
...
function ENT:setup_gearbox(vehicleMode)
	...
	if enableTracks[vehicleMode] then self:setup_tracks(vehicleMode) end
end
...
function ENT:update_gearbox(vehicleMode, selfTbl)
	if not vehicleMode or not selfTbl.dak_isVisible then return end
	selfTbl.dak_isVisible = nil
...
local function calcbounds(min, max, pos)
	if pos.x < min.x then
		min.x = pos.x
	elseif pos.x > max.x then
		max.x = pos.x
	end

	if pos.y < min.y then
		min.y = pos.y
	elseif pos.y > max.y then
		max.y = pos.y
	end

	if pos.z < min.z then
		min.z = pos.z
	elseif pos.z > max.z then
		max.z = pos.z
	end
end
...
local function setup_modelinfo(self, vehicleMode, dak_wheels_info)
	...
	for _, info in pairs(dak_wheels_info) do
		if IsUselessModel(info.model) or not file.Exists(info.model, "GAME") then info.model = "models/hunter/blocks/cube025x025x025.mdl" end
		...
		if hmin and hmax then hbb = hmax - hmin end
		...
		for char = 1, #bodygroup do
			...
			if isnum then bodygroup = string.format("%s%s%s", string.sub(bodygroup, 1, char - 1), isnum, string.sub(bodygroup, char + 1)) end
		end
	...
	if #material == 1 then
		...
		for i = 1, #material, 2 do
			...
			if id and material[i + 1] then dak_csents_wheel:SetSubMaterial(id, material[i + 1]) end
		end
	end
end
	...
	if winfo then
		...
		for i = 0, num_road - 1 do
			...
			addwheel(self.dak_wheels_ri, {
				...
				posL = pos + pos_ri,
				posA = pos + pos_ri,
				angL = Angle(0, alt and 0 or 180, 0),
				angA = Angle(0, alt and 0 or 180, 0)
			})
			...
			addwheel(self.dak_wheels_le, {
				...
				posL = pos + pos_le,
				posA = pos + pos_le,
				angL = Angle(0, alt and 180 or 0, 0),
				angA = Angle(0, alt and 180 or 0, 0)
			})
...
function ENT:update_wheels(vehicleMode)
	...
	if math_abs(bias_fo) < 0.01 then bias_fo = nil end
	local bias_ri = self.dak_wheels_bias_ri * trace_length
	if math_abs(bias_ri) < 0.01 then bias_ri = nil end
	...
	if cv_part:GetBool() then
		...
		if fxscale_ri == 0 then fxscale_ri = nil end
		fxscale_le = math_min(effectmax, math_abs(vel_le * 0.5))
		if fxscale_le == 0 then fxscale_le = nil end
	end

	if debugmode and debugtype.trace.enabled then debugtype.trace:update(self, true) end
...
function ENT:render_wheels(vehicleMode)
	...
	for i = vehicleMode == "halftracked" and 0 or 1, selfTbl.dak_wheels_count_ri do
		...
		if prevtype ~= wheel_ri.type or prevalt ~= wheel_ri.alt then
			...
			if prevalt ~= wheel_ri.alt then prevalt = wheel_ri.alt end
...
local function createTrackTexture(path)
	...
	for k, v in pairs(folders) do
		local check = v .. path
		if track_textures[check] then return track_textures[check] end
	...
	if not diffuse then diffuse = "hunter/myplastic" end
	local shader = {
		["$basetexture"] = diffuse,
		["$alphatest"] = "1",
		["$nocull"] = "1",
...
local trackverts_model = function(y, z) return {Vector(0, y * 0.5, 0), Vector(0, -y * 0.5, 0), Vector(0, y * 0.375, z * 0.5), Vector(0, -y * 0.375, z * 0.5), Vector(0, y * 0.375, -z * 0.5), Vector(0, -y * 0.375, -z * 0.5), Vector(0, 0, z * 0.5), Vector(0, 0, -z * 1.5)} end
local _angle = Angle()
...
function ENT:update_tracks(vehicleMode)
	...
	local tension_val = selfTbl.dak_tracks_tension
	local tension_dir = selfTbl.dak_tracks_tensor
	for i = 1, 2 do
		local trackroots, trackrootscount, tracknodes, tracknodesdetail
		if i == 1 then
			trackroots = selfTbl.dak_wheels_ri
			trackrootscount = selfTbl.dak_wheels_count_ri
			tracknodes = selfTbl.dak_tracks_nodes_ri
			tracknodesdetail = adaptive_detail and (min_detail + math_floor(math_abs(selfTbl.dak_wheels_lastvel_ri) * 2)) or max_detail
		else
			trackroots = selfTbl.dak_wheels_le
			trackrootscount = selfTbl.dak_wheels_count_le
			tracknodes = selfTbl.dak_tracks_nodes_le
			tracknodesdetail = adaptive_detail and (min_detail + math_floor(math_abs(selfTbl.dak_wheels_lastvel_le) * 2)) or max_detail
		end

		if tracknodesdetail > max_detail then tracknodesdetail = max_detail end
		local tracknodesdetailrad = 1 / (45 / tracknodesdetail)
		local tracknodescount = 0
		local switchdir = -1
		for rootid = 1, trackrootscount do --This part is expensive
			...
			if (dir2.x > 0) ~= (dir0.x > 0) then count = math_Round(math_abs(atan2) - math_abs(atan0)) * tracknodesdetailrad end
			if count > 0 then
				for k = 0, count do
		...
		if switchdir > 0 then
			...
			if tension_val > 0 then tracknodescount = tracknodescount + bisect_spline(tracknodescount, tracknodes, tension_dir, tension_det, tension_rad) end
		else
			...
			for n = 1, splitcount - 1 do
				...
				if n < splitcount then
					...
					if diffz < radius then nextz = nextz + (radius - diffz) end
					node1.z = nextz
				end
		...
		if i == 1 then
			selfTbl.dak_tracks_nodescount_ri = tracknodescount
		else
			selfTbl.dak_tracks_nodescount_le = tracknodescount
		end
	end

	local trackmodel = selfTbl.dak_tracks_model
	local trackmodelcount = selfTbl.dak_tracks_modelcount
	local trackresolution = selfTbl.dak_tracks_texturemap
	for i = 1, 2 do
		...
		if i == 1 then
			...
			tracknormals = selfTbl.dak_tracks_normals_le
		end
		...
		if i == 1 then
			selfTbl.dak_tracks_vertscount_ri = trackvertscount
		else
			selfTbl.dak_tracks_vertscount_le = trackvertscount
		end
	end

	selfTbl.dak_tracks_ready = true
end
...
function ENT:render_tracks(vehicleMode)
	...
	local detail = trackDetail:GetInt()

	cam.PushModelMatrix(self:parentMatrix_Get())
	local texture = selfTbl.dak_tracks_texture
	if texture and selfTbl.dak_tracks_color then texture:SetVector("$color2", selfTbl.dak_tracks_color) end
	for i = 1, 2 do
		...
		if i == 1 then
			...
			tracknormals = selfTbl.dak_tracks_normals_le
			trackscroll = selfTbl.dak_wheels_lastrot_le
		end

		if texture then texture:SetVector("$newscale", i == 1 and flip1 or flip2) end
		render_SetMaterial(texture or matfallback)
		mesh_Begin(MATERIAL_QUADS, tracknodescount * 7 + 7)
		local ytrans = trackscroll or 0
		local yshift = 0
		for nodeid = 1, tracknodescount do --TODO: This is expensive. Not sure if I can improve it much, but worth noting.
			...
			--I'm fully aware checking detail repeatedly like this isn't optimal, but I'm making a very minor performance concession for readability,
			--and because doing something smarter would take more effort, and I'd rather spend that effort on improving other parts of the code.
